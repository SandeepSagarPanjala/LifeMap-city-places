...
Rough scale: **a few cities per state** (often ~2–8), not hundreds. Worldwide: expand **country by country**; United States, India, and Japan are complete — next is **United Kingdom**, then the queue in [README.md](./README.md). Each babysit session should **finish one country then immediately start the next** (not a partial state slice; do not ask between countries).
